...
def render(data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        *([f"- 部分估算：{pct(portfolio.get('partial_weekly_return'))}（净值覆盖率不足，仅供参考）"] if (portfolio.get("nav_coverage_weight") or 0) < 0.9 else []),
        "",
        "## 三周综合判断",
        "",
        ...
        "",
        table(
            ["阶段", "余额峰值", "峰值日", "当前距峰值", "杠杆密度峰值", "交易强度峰值", "20日最快扩张", "峰后20日宽基指数最大回撤", "峰后60日宽基指数最大回撤"],
        ...
        *[f"- {item}" for item in conclusion_lines if item],
        ...
        table(
            ["类型", "板块", "今日涨跌", "数据日期", "口径"],
            [[row.get("type"), row.get("name"), pct(row.get("today_return")), row.get("source_date") or "-", row.get("return_basis")] for row in (sectors.get("industry_today") or [])[:5] + (sectors.get("concept_today") or [])[:5]],
        ),
        f"### 报告期后当日资金观察（{week.get('collection_trade_date')}，不参与报告周结论）",
        "",
        table(
        ...
        f"### 报告期5日行业资金流出 Top10（截至 {week.get('end_date')}）",
        "",
        table(["板块", "行业分类", "收益", "单日资金", "5日资金", "10日资金", "资金状态", "判定依据", "组合相关主题估算占比", "分类依据/置信度", "收益口径", "截止日", "缓存天数", "资金口径", "样本范围", "候选ETF"], sector_rows(sectors.get("industry_outflow") or [])),
        f"### 报告期5日概念资金流出 Top10（截至 {week.get('end_date')}）",
        "",
        table(["板块", "行业分类", "收益", "单日资金", "5日资金", "10日资金", "资金状态", "判定依据", "组合相关主题估算占比", "分类依据/置信度", "收益口径", "截止日", "缓存天数", "资金口径", "样本范围", "候选ETF"], sector_rows(sectors.get("concept_outflow") or [])),
        "### 基金主题热度代理（非板块收益）",
        "",
        table(
        ...
        "## 近1周基金排行信号",
        "",
        f"_排行为采集日（{week.get('collection_trade_date') or '-'}）的近1周快照，不是报告周净值口径；只作信号，不作执行依据。_",
        "",
        table(
        ...
        table(
            ...
            [
                [
                    ...
                    " / ".join("-" if (row.get("live_snapshot") or {}).get(key) is None else str((row.get("live_snapshot") or {}).get(key)) for key in ("price", "iopv")),
