...
项目已在 Codex 和 Claude Code 中开发、调试和审查，并提供 `SKILL.md` 与可选的 `agents/openai.yaml`；核心采集、分析、渲染和验证均为普通 Python CLI，不依赖 Codex 运行时。任何能够执行命令、读写 JSON 并展示 HTML 的 Agent 都可以调用，也可以只把它当作本地分析工具使用。

2026-09 审查更新：整合本地 Agent 对净值、持仓主题和两融图表的改动，补齐历史截止日、缺日净值、排行评分与分榜展示的回归检查。升级后旧版推算净值缓存会重新核验；排行快照仅作观察信号，需补齐报告期净值后才能参与替换评分。
